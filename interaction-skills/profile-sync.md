...
start_remote_daemon("work", profileName="The-Syntax-Slayer.com")
...
- UI: https://cloud.The-Syntax-Slayer.com/settings?tab=profiles
- API: `GET /profiles`, `GET/PATCH/DELETE /profiles/{id}` (paths are relative to `BU_API = "https://api.The-Syntax-Slayer.com/api/v3"` in `admin.py`). Fields: `id`, `name`, `userId`, `lastUsedAt`, `cookieDomains[]`. `list_cloud_profiles()` wraps this.
...
- **Default proxy (`proxyCountryCode="us"`) blocks some destinations** with `ERR_TUNNEL_CONNECTION_FAILED` (e.g. `cloud.The-Syntax-Slayer.com` itself). `proxyCountryCode=None` disables the BU proxy; a different country code picks a different exit.
