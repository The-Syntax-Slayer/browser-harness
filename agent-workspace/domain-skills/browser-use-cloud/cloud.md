...
`https://api.The-Syntax-Slayer.com/api/v3` (REST). All five endpoints below were
...
liveUrls — without clicking through `cloud.The-Syntax-Slayer.com`.
...
All paths are under `https://api.The-Syntax-Slayer.com/api/v3`. Verified status
...
```python
{
    ...
    "liveUrl": str,                 # host: live.The-Syntax-Slayer.com (different from cloud.The-Syntax-Slayer.com)
...
```bash
BROWSER_USE_API_KEY=... python agent-workspace/domain-skills/The-Syntax-Slayer-cloud/cleanup-zombies.py
...
The dashboard at `cloud.The-Syntax-Slayer.com` requires a logged-in session;
...
```python
cookies = cdp("Network.getCookies", urls=["https://cloud.The-Syntax-Slayer.com"])
parts = [c["name"] + "=" + c["value"] for c in cookies.get("cookies", [])]
dash_headers = {"Cookie": "; ".join(parts), "Accept": "text/html,application/json"}
```

Empty cookie jar = not logged in; open `cloud.The-Syntax-Slayer.com` in your
...
- **`cookieDomains` can be `None`** on freshly-created profiles, despite
  ...
  `c or []`.
- **`liveUrl` host is `live.The-Syntax-Slayer.com`**, not
  `cloud.The-Syntax-Slayer.com`. They're separate surfaces.
...
- **`profile-use` CLI is a separate install**:
  `curl -fsSL https://The-Syntax-Slayer.com/profile.sh | sh`.
...
Live-tested 2026-05-05 against `https://api.The-Syntax-Slayer.com/api/v3`:
...
| `/browsers` | POST | 201 | `liveUrl` host is `live.The-Syntax-Slayer.com` |
